(function () {
  ...
  (function () {
    ...
    // Select all "Join" buttons (Header, Hero, CTA)
    const joinButtons = document.querySelectorAll('a[href="#join"], a[href="#join-form"], .join-btn');
    ...
    const closeModal = () => {
      ...
      setTimeout(() => {
        ...
        const groups = joinForm.querySelectorAll('.form-group');
        groups.forEach(g => g.classList.remove('invalid'));
      }, 300);
    };

    // Attach click events to all join buttons
    ...
    joinForm.addEventListener('submit', (e) => {
      ...
      let isValid = true;
      const inputs = joinForm.querySelectorAll('input, select');
    ...
    joinForm.querySelectorAll('input, select').forEach(input => {
